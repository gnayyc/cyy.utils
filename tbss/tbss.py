...
vectors = ["FA", "L1", "L2", "L3", "V1", "V2", "V3", "MD", "MO", "S0"]
...
if not os.path.exists(os.path.join(TBSS_dir,'.tbss.done')): 
    print "########"
    print "######## TBSS"
    print "######## - copy data from groups_dir into TBSS_dir"
    ...
    if not os.path.exists(os.path.join(TBSS_dir,'.tbss.fa.done')): 
        ...
        if not os.path.exists(os.path.join(TBSS_dir,'.tbss.fa.randomise.done')): 
            ...
            #print
            commands.getoutput(' '.join([randomise, '-i', 'all_FA_skeletonised', '-o',
                'tbss', '-m', 'mean_FA_skeleton_mask', '-d', 'design.mat',
                '-t', 'design.con', '-n', '500', '--T2', '-V']))
            os.rename('tbss_tfce_corrp_tstat1.nii.gz',
                'tbss_tfce_corrp_tstat1_FA.nii.gz')
            os.rename('tbss_tfce_corrp_tstat2.nii.gz',
                'tbss_tfce_corrp_tstat2_FA.nii.gz')
            os.rename('tbss_tfce_p_tstat1.nii.gz',
                'tbss_tfce_p_tstat1_FA.nii.gz')
            os.rename('tbss_tfce_p_tstat2.nii.gz',
                'tbss_tfce_p_tstat2_FA.nii.gz')
            ...
            print "fslview $FSLDIR/data/standard/MNI152_T1_1mm mean_FA_skeleton -l Green -b 0.2,0.8 tbss_tstat1 -l Red-Yellow -b 3,6 tbss_tstat2 -l Blue-Lightblue -b 3,6"
            print
    ...
    for v in vectors[1:]:
        ...
        if not os.path.exists(os.path.join(TBSS_dir,'.tbss.'+v+'.randomise.done')): 
            ...
            os.rename('tbss_tfce_p_tstat1.nii.gz',
                'tbss_tfce_p_tstat1_.nii'+v+'.gz')
            os.rename('tbss_tfce_p_tstat2.nii.gz',
                'tbss_tfce_p_tstat2_.nii'+v+'.gz')
